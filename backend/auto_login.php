	...
	if($user[0]==$token){	//可以登录
		...
		switch($place)
		{
			...
			case 'student_class_login': //学生选择课程以后的登陆界面 返回 当前已选课程 当前进入课程的作业 通知情况
				...
				$sql="select class_note,class_work_name,class_work_type,class_work_size,class_work_deadline from classes where class_id='$class_id'";
				$result = mysqli_query($con,$sql);
				...
				'class_work_size'=>$return_info1[3],'class_work_deadline'=>$return_info1[4]);
				break;
		/*	case 'quiz':
